...
var WaveSurfer = {
    ...
    playExtended: function (start, loop, end, render, index, loopback) {
        this.backend.playExtended(start, loop, end, render, index, loopback);
    },	
	...
	setSpeed: function(t) {
		...
		this.speed = t;
		if(this.stopstart) {
			if(this.fromLoop) this.seekToAndPlayAll(this.getCurrentTime()/this.getDuration());
			else this.seekToAndPlaySingle(this.getCurrentTime()/this.getDuration(), false, false, 0);
			this.stopstart = false;
		}
    ...
    seekToAndPlaySingle: function(progress, fromLoop, render, index, loopback) { 
   		var my = this;
		...
		this.playExtended(this.activeSliceMarkers[0] * this.getDuration(), this.activeSliceMarkers[1] * this.getDuration(), (this.activeSliceMarkers[0] + playableSlice) * this.getDuration(), render, index, loopback); 
		my.fromLoop = fromLoop;
		this.once('pause', function() {
			if(my.fromLoop == false) {
				my.seekAndCenter(my.activeSliceMarkers[1]);	
			}
		});		
    },	
	...
	seekToAndPlayAll: function(progress) {
		this.seekToAndPlaySingle(progress, true, false, 0, 0);
		var self = this;
		this.on('pause', function() {
			self.seekToAndPlaySingle(self.activeSliceMarkers[1], true, false, 0, 1);  
		});
	},
	...
	seekToAndRenderAll: function(filename) {
		...
		this.seekToAndPlaySingle(progress + 0.0000000001, false, true, this.segmentIndex, 0);
		this.on('render', function() {		
			if(my.activeSliceMarkers[1] == my.getRightLocator())  {  
				my.filemaker.pushwav(my.getSlices());
				my.un('render');
				my.fireEvent('finished-render');
			}
			else {
				my.segmentIndex = my.segmentIndex  + 1;
				my.seekToAndPlaySingle(my.activeSliceMarkers[1] + 0.0000000001, false, true, my.segmentIndex, 0);
			}
		});
	},
	...
	freeBuffers: function() {
		this.backend.freeBuffers();
	},	
	

    setBand: function(band, setting) {
 		this.backend.fftwolaL.setBand(band, setting);	
		this.backend.fftwolaR.setBand(band, setting);	   
    },   
    ...
    empty: function () {
        ...
        this.backend.fftwolaL.flushMemory();	
        this.backend.fftwolaR.flushMemory();	
    },
